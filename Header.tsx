      ...
      <nav className="container-custom">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex-shrink-0">
            <h1 className="text-2xl font-bold gradient-text">SHV Dandu</h1>
          </div>
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 bg-white/95 backdrop-blur-lg rounded-lg shadow-lg mt-2">
            {navItems.map((item) => (
              <button
                key={item.href}
                onClick={() => scrollToSection(item.href)}
                className="text-gray-700 hover:text-blue-600 hover:bg-blue-50 block w-full text-left px-3 py-2 text-base font-medium rounded-lg transition-colors duration-200"
              >
                {item.label}
              </button>
            ))}
